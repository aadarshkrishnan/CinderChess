...
namespace chess {
    ...
    bool Knight::Move(int new_x_position, int new_y_position, Board board) {
    ...
    bool Knight::CheckPossibleMove(int new_x_position, int new_y_position, Board board) {
        if (board.getBoard()[new_x_position][new_y_position].getColor() != board.getBoard()[current_position_x_][current_position_y_]) {
            return true;
        }
    }
}